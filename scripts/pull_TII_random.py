#!/usr/bin/env python3
"""
pull_TII_random.py - Script to pull data from TII endpoint with language and category filters

This script fetches task data from the TII crowdlabel API endpoint
with specific filters for language and category, and prints it to the console.
The script can also submit the fetched tasks to the hotlabel-tasks service.

Usage:
    python pull_TII_random.py
    python pull_TII_random.py --api-key YOUR_API_KEY
    python pull_TII_random.py --verify  # Only use this if you want SSL verification
"""
...
# Configuration
TII_API_ENDPOINT = "https://crowdlabel.tii.ae/api/2025.2/tasks/pick?lang=en&category=vqa"
...
def fetch_tii_tasks(api_key, verify_ssl=False):
    """
    Fetch tasks from the TII endpoint with language and category filters
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description="Fetch task data from TII endpoint with language and category filters")
...
def main():
    ...
    logger.info("Starting TII random data pull script")
    ...
    logger.info("TII random data pull completed")
